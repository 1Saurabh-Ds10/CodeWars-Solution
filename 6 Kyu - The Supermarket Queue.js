...
function queueTime(customers, n) {
  ...
  for (let i=0, l = customers.length; i<l; i++) {
      ...
      queues[queues.indexOf(Math.min(...queues))] += customers[i];
  }
...
function queueTime(customers, n) {
  ...
  for (let i=0, l = customers.length; i<l; i++) {
    ...
    let q = queues[0].reduce((a,b) => a+ b, 0), slot = 0;
    for (j =0; j<n; j++) {
      let currQ = queues[j].reduce((a,b) => a+ b, 0);
      if (currQ < q) {
        q = currQ;
        slot = j;
    ...
    queues[slot].push(customers[i]);
  }
  
  return Math.max.apply(this, queues.map(v => v.reduce((a,b) => a + b, 0)));
