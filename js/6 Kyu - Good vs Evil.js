...
function goodVsEvil(good, evil){
  const goodW = [1,2,3,3,4,10],
        evilW = [1,2,2,2,3,5,10];
 
  
  const calW = (str, worth) => str.split(' ').map((w, i) => +w * worth[i]).reduce((a,b) => a + b, 0);
  const calG = calW(good, goodW);
  const calE = calW(evil, evilW);
  
  
  return ((calG > calE) ? 'Battle Result: Good triumphs over Evil' :
          
          (calG === calE) ? 'Battle Result: No victor on this battle field' :
